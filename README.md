...
Open `http://localhost:5000` → **Job Hunt Agent** tab → **Upload Files** panel.
- Resume: **.docx only** (tailoring edits the docx XML directly — a PDF can't be tailored).
- Cover letter: PDF or DOCX — used as the tailoring template.

Both land in `uploads/resume/` (`resume.docx`, `cover_letter.*`). Don't place files there manually —
upload through the panel so the dashboard's status indicators and role-extraction cache stay in sync.

On Step 4 (Select), ticking **Tailor Resume** for a job generates both English and German versions of
the resume and cover letter, tailored to that job, in `uploads/resume/tailored/<company>/`:
`resume_en`, `resume_de`, `cover_letter_en`, `cover_letter_de` (each as `.docx` + `.pdf`). The German
versions are a full rewrite (not a patch of the English one — python-docx can't reliably translate text
in place), generated by Claude from the same base resume/cover letter. The applier picks the file
matching the job's detected language automatically, preferring PDF (what actually gets sent) over docx.
One folder per company — re-tailoring the same company overwrites its previous version.
...
├── uploads/
│   ├── resume/
│   │   ├── resume.docx      # base resume — upload via dashboard's Upload Files panel
│   │   ├── cover_letter.*   # base cover letter template — upload via the same panel
│   │   └── tailored/
│   │       └── <company>/   # resume_en/de + cover_letter_en/de (.docx + .pdf), generated by tailor.py
│   ├── resume_de.pdf        # optional German resume — falls back to EN if absent
