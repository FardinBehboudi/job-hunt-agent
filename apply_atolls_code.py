"""
Enter verification code Yet8mbO3 into the Atolls Greenhouse form and submit.
The form was already filled — just need to enter the code and resubmit.
"""
...
async def main():
    print(f"=== ATOLLS — Entering code: {CODE} ===")
    async with async_playwright() as pw:
        ...
        # First submit — triggers verification code prompt
        sub = await frame.query_selector("input[type='submit'], #submit_app")
        if sub:
            print("\n  → Submit #1 (to trigger code prompt)...")
            await sub.scroll_into_view_if_needed(timeout=3000)
            await sub.click()
            await page.wait_for_timeout(6000)
            await page.screenshot(path=f"uploads/code_after_submit1_{ts()}.png")

        # Look for the verification code input field
        print(f"\n  → Looking for code input field...")
        code_inp = None
        for code_sel in [
            "input[name*='code' i]",
            "input[id*='code' i]",
            ...
            # Greenhouse code field is often just a plain text input after the submit section
            "#security_code",
            ...
            try:
                ...
                if el:
                    ...
                    print(f"    Found code input: {code_sel} visible={vis}")
                    if vis:
                        code_inp = el
                        break
            except Exception: pass
        ...
        if not code_inp:
            # Find any NEW input that appeared after submit
            print("    Scanning all visible inputs for code field...")
            all_inp = await frame.query_selector_all("input[type='text'], input[type='number']")
            for inp in all_inp:
                try:
                    if await inp.is_visible():
                        id_a = await inp.get_attribute("id") or ""
                        ph = await inp.get_attribute("placeholder") or ""
                        print(f"    visible input: id={id_a!r} ph={ph!r}")
                        if any(k in (id_a+ph).lower() for k in ["code","verif","token","security"]):
                            code_inp = inp
                            break
                except Exception: pass
        ...
        if code_inp:
            ...
            await page.screenshot(path=f"uploads/code_entered_{ts()}.png")

            # Final submit
            sub2 = await frame.query_selector("input[type='submit'], #submit_app, button[type='submit']")
            if sub2:
                print("  → Final submit...")
                await sub2.click()
                await page.wait_for_timeout(10000)
                await page.screenshot(path=f"uploads/code_final_{ts()}.png")
                ...
                ok = any(k in combined for k in [
                    "thank you","application received","successfully",
                    "submitted","we received","thanks for applying","your application"
                ])
                print(f"  Frame body: {frame_body[:400]}")
                print(f"\n  {'✅ SUCCESS — Atolls application submitted!' if ok else '❌ FAILED'}")
        else:
            print("  ❌ Could not find code input field")
            await page.screenshot(path=f"uploads/code_no_field_{ts()}.png")
